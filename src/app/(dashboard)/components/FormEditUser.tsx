...
import { updateUser } from '@/app/(dashboard)/data/action';
...
export default function FormEditUser({ user }: { user: any }) {
  ...
  const [previewImage, setPreviewImage] = useState<string | null>(user.image ? JSON.parse(user.image).imageUrl : null);
  ...
  return (
    <form action={dispatch}>
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <ButtonPrimary type="submit">Update User</ButtonPrimary>
      </div>
    </form>
  );
}
